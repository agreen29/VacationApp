#' The make_output fucntion greats a dataframe that includes weather forecasting for up to 15 days from today, after a user inputs 
#' a starting data and ending date of a trip. Th funciton uses another funciton in this package to get an address and plugs it into
#' a query for a weather API to collect the forecast on that loaction. 
#'
#' @importFrom httr GET
...
#' This is a helper function that takes in dates in "yyyy-mm-dd" format and outputs them in a form that the API can handle: 
#' @importFrom anytime anydate 
#' 
#' @param day_leaving the date the user is planning to leave for their vaction in "yyyy-mm-dd" format
#' @param day_returning the last date the user is planning to be on their vaction in "yyyy-mm-dd" format
#' 
...
#' This is the function that calls the API with the given dates from the date_input fucntion and the location from the 
#' directions function. 
#' 
#' @importFrom httr GET
#' @importFrom jsonlite fromJSON
...
#'this function uses the list of lists from the returned element in query_weather and creates a dataframe to show all weather 
